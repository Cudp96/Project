...
const Sidebar = () => {
  ...
  const activeLink = ' flex rounded-sm gap-2 text-white font-extrabold bg-orange-500 py-4 px-2'
  const inActiveLink = 'flex gap-2 p-1 hover:bg-orange-400 py-4 px-2 '

  const {pathname} = useRouter();

  return (
    <aside className=" hidden  md:block flex flex-col  bg-orange-300 min-h h-screen w-1/5">
      <div>
        <Logo />
      </div>
      ...
      <div>
        <button className="flex flex-row fixed left-4 bottom-5 hover:bg-orange-400 py-4 px-2 rounded-2xl" onClick={() => signOut()}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
